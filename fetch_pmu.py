#!/usr/bin/env python3
"""
STATO Live — Collecteur de données PMU v2.2
Filtre automatique : courses françaises uniquement
"""
...
# Hippodromes français reconnus (noms DB STATO)
HIPPOS_FR = {
    "Vincennes", "Auteuil", "Paris-Longchamp", "Deauville", "Chantilly",
    "Saint Cloud", "Cagnes Sur Mer", "Enghien Soisy", "Enghien",
    "Maisons Laffitte", "Compiegne", "Cabourg", "Clairefontaine",
    "Vichy", "Pau", "Fontainebleau", "Caen", "Laval", "Lyon Parilly",
    "Marseille Borely", "Angers", "Craon", "Nantes", "Toulouse", "Dieppe",
    # Hippodromes français supplémentaires hors base historique
    "La Teste De Buch", "Bordeaux Le Bouscat", "Argentan", "Royan La Palmyre",
    "Brest", "Rennes", "Mauquenchy", "Beaumont De Lomagne", "Reims",
    "Chateaubriant", "Amiens", "Strasbourg", "La Capelle",
}

# Mapping noms API → noms DB
HIPPO_MAP = {
    ...
    "LA TESTE-DE-BUCH": "La Teste De Buch", "LA TESTE DE BUC": "La Teste De Buch",
    "BORDEAUX-LE-BOUSCAT": "Bordeaux Le Bouscat",
    "ARGENTAN": "Argentan", "ROYAN": "Royan La Palmyre",
    "BREST": "Brest", "RENNES": "Rennes",
    "MAUQUENCHY": "Mauquenchy",
    "BEAUMONT-DE-LOMAGNE": "Beaumont De Lomagne",
    "REIMS": "Reims", "CHATEAUBRIANT": "Chateaubriant",
    "AMIENS": "Amiens", "STRASBOURG": "Strasbourg",
    "LA CAPELLE": "La Capelle",
}

def norm_hippo(raw):
    """Normalise le nom brut API → nom DB STATO."""
    ...
    # Recherche partielle sur les clés
    for k, v in HIPPO_MAP.items():
        if k in up or up in k:
            return v
    # Si commence par "Hippodrome De" → étranger
    if 'HIPPODROME DE' in up:
        return None
    # Title case comme fallback (peut être un hippo FR inconnu)
    tc = ' '.join(w.capitalize() for w in raw.split())
    return tc if tc else None

def is_french(hippo_norm):
    """Vérifie si l'hippodrome est français."""
    if not hippo_norm: return False
    # Vérifie correspondance exacte ou partielle avec HIPPOS_FR
    if hippo_norm in HIPPOS_FR: return True
    for h in HIPPOS_FR:
        if h.lower() in hippo_norm.lower() or hippo_norm.lower() in h.lower():
            return True
    return False
...
def get_hippo_raw(r):
    """Extrait le nom brut de l'hippodrome depuis tous les champs possibles."""
    h = r.get('hippodrome') or {}
    for field in ['libelle', 'libelleLong', 'libelleCourt', 'nom', 'libelle']:
        v = h.get(field)
        if v and v.strip(): return v.strip()
    for field in ['libelle', 'libelleLong', 'libelleCourt']:
        v = r.get(field)
        if v and v.strip(): return v.strip()
    return ''
...
def extraire_partant(p):
    ...
    return {
        ...
        'coteTendance': tendance, 'favori': favori,
        ...
        'oeilleres': oeilleres, 'deferre': deferre,
...
def fetch_programme(date_str):
    ...
    print(f"  {len(reunions_raw)} reunion(s) au total")
    reunions = []
    mapping_log = []
    for r in reunions_raw:
        raw = get_hippo_raw(r)
        hippo = norm_hippo(raw)
        fr = is_french(hippo)
        mapping_log.append({'brut': raw, 'normalise': hippo or '?', 'france': fr})
        print(f"  {'✓' if fr else '✗'} '{raw}' → '{hippo}'")
        if not fr:
            continue  # Ignorer les hippodromes étrangers
        ...
        for c in r.get('courses', []):
            ...
            if p_data:
                for p in p_data.get('participants', []):
                    pt = extraire_partant(p)
                    if pt: course['participants'].append(pt)
                nb = len(course['participants'])
                nb_c = sum(1 for p in course['participants'] if p['cote'])
                print(f"    C{num_c} {nb} partants | {nb_c} cotes")
            courses.append(course)
        reunions.append({'num': num_r, 'hippo': hippo, 'courses': courses})
    print(f"  → {len(reunions)} reunion(s) francaises retenues")
    return reunions, mapping_log

def fetch_arrivees(date_str, france_only=True):
    print(f"Arrivees {date_str} {'(France uniquement)' if france_only else ''}")
    data = fetch(BASE_PROG.format(date=date_str))
    if not data: return []
    arrivees = []
    for r in data.get('programme', {}).get('reunions', []):
        raw = get_hippo_raw(r)
        hippo = norm_hippo(raw)
        if france_only and not is_french(hippo):
            continue
        num_r = r.get('numOfficiel', 0)
        for c in r.get('courses', []):
            num_c = c.get('numOrdre', 0)
            ...
            if nums:
                arrivees.append({
                    ...
                    'lieu': hippo or raw, 'rc': f"R{num_r}C{num_c}",
...
def main():
    ...
    print(f"STATO Live v2.2 — {now.strftime('%d/%m/%Y %H:%M')}")
    programme, mapping_log  = fetch_programme(today)
    arrivees = fetch_arrivees(yesterday, france_only=True)
    ...
    output = {
        ...
        'stats':          {
            'reunions': len(programme),
            'partants': total_part,
            'avec_cotes': total_cotes,
        },
        'hippos':         mapping_log,  # Toujours présent pour vérification
    }
    with open('live.json', 'w', encoding='utf-8') as f:
        json.dump(output, f, ensure_ascii=False, separators=(',', ':'))
    kb = os.path.getsize('live.json') // 1024
    print(f"OK — live.json {kb} KB | {len(programme)} reunions FR | {total_part} partants | {total_cotes} cotes")
    sys.exit(0)
