#!/usr/bin/env python3
"""
STATO Live — Collecteur de données PMU v2
...
def extraire_partant(p):
    ...
    avis_court = avis_map.get(p.get('avisEntraineur', 'NEUTRE'), '=')
    return {
        'num':            p.get('numPmu'),
        'nom':            p.get('nom', '?'),
        'driver':         p.get('driver', ''),
...
def main():
    ...
    print(f"STATO Live v2 — {now.strftime('%d/%m/%Y %H:%M')}")
